...
#define SIZE 512
#define RA "-a"                             // Requirement a
#define RB "-b"                             // Requirement b
#define RC "-c"                             // Requirement c
...
void fft_2d(complex img[][SIZE], int isign);
void MM_Point(complex img1[][SIZE], complex img2[][SIZE], complex out[][SIZE]);
...
int main(int argc, char** argv) 
{
    ...
    fft_2d(img_1, -1);             // 2d fft
    fft_2d(img_2, -1);             // 2d fft
    MM_Point(img_1, img_2, out);   // Point product
    fft_2d(out, 1);                // Inverse 2d fft
...
/* Use 2D fft with MPI send and recv*/
void fft_2d(complex img[][SIZE], int isign)
{
    ...
    communicate();
    // Scatter with MPI_Send & Recv.
    if(proc_rank == 0) { 
        for(i = 1; i < proc_num; i++) {
            MPI_Send(&img[i * chunk_size][0], chunk_size, row_type, i, 100, MPI_COMM_WORLD);
        }
    } else {
        MPI_Recv(img, chunk_size, row_type, 0, 100, MPI_COMM_WORLD, &status);
    }
    communicate();
    ...
    communicate();
    // Gather with MPI_Recv.
    if(proc_rank == 0) { 
        for(i = 1; i < proc_num; i++) {
            MPI_Recv(&img[i * chunk_size][0], chunk_size, row_type, i, 100, MPI_COMM_WORLD, &status);
        }
        transpose(img);
    } else {
        MPI_Send(img, chunk_size, row_type, 0, 100, MPI_COMM_WORLD);
    }
    communicate();
